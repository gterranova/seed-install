    ...
    return gulp.watch([`../${SRC}**/*.common.ts`, `../${SRC}**/*.tns.ts`, `../${SRC}**/*.tns.html`,  `../${SRC}**/*.service.ts`,
     ...
     `../${SRC}**/index.ts`, `../${SRC}**/*.index.ts`])
        .on('change', (file) => {
            ...
            var outputDest = file.replace('/', '\\');
            outputDest = outputDest.substring(0, outputDest.lastIndexOf('\\')+1).replace(SRC.substring(0, SRC.length - 1), DEST.substring(0, SRC.length - 1)).replace('..\\', '.\\');
            outputDest = outputDest.substring(0, outputDest.length - 1);
            var outputFile = outputDest + file.replace('/', '\\').substring(file.lastIndexOf('\\')).replace('.tns', '');
            ...
            gulp.src([file])
                ...
                .pipe(replace('.scss\'', '.css\'', { logs: { enabled: false }}))
                .pipe(debug({title: 'tns.Livesync'}))
                .pipe(debug({title: 'out ' + outputFile}))
...
gulp.task('tns.Build.Android', (cb) => {
    var buildProcess = exec('tns run android', { maxBuffer: 10000*1024 }, function (err, stdout, stderr) {
        console.log(stdout);
        console.log(stderr);
        cb(err);
      });
...
gulp.task('build.cli.Live', (cb) => {
    // task1 and task2 will run in different processes
    return gulpMultiProcess(['tns.Build.Android', 'tns.Livesync'], cb);
});